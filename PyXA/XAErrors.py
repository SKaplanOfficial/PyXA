...
class AppleScriptError(Exception):
	...
	def __init__(self, err: dict, script: str):
		...
		error_context = script[script.rfind("\n", 0, error_range.location):script.find("\n", error_range.location, len(script))].strip()

		print(script.split("\n"))
		error_line = next((n for (n,l) in enumerate(script.split("\n")) if error_context in l), -1)
		...
		Exception.__init__(self, f"Error {self.number}: {self.message} On line #{self.line_number}: '{self.near}'.")
